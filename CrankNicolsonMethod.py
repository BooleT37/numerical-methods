...
class CrankNicolsonMethod(GridMethod):
    def __init__(self, n, m, x_max, t_max):
        super().__init__(n, m, x_max, t_max)
        self.coeff = self.tau / (self.h ** 2)
        self.a = self.c = -self.coeff
    ...
    def count_d(self, u, i, j):
        return self.coeff * (u[j - 1, i - 1] - 2 * u[j - 1, i] + u[j - 1, i + 1]) + \
               self.tau * count_g(i, j - 1, self.h, self.tau) + \
               u[j - 1, i]
